...
export const VOCABULARY = {
  ...
  ward_structure: [
    ...
    'male', 'female', 'active', 'chronic', 'new', 'pending', 'list',
    'follow up', 'follow-up', 'stable', 'unstable', 'critical',
    'improving', 'deteriorating', 'worsening', 'resolved', 'ready',
  ...
  connectors: [
    'and', 'with', 'without', 'of', 'on', 'for', 'to', 'from', 'in',
    'due', 'secondary', 'versus', 'vs',
  ],
};
...
export function getColumnVocabulary(columnType) {
  ...
  const connectors = new Set(VOCABULARY.connectors.map(w => w.toLowerCase()));

  if (type.includes('patient') || type.includes('name')) {
    return new Set([
      ...VOCABULARY.names.map(w => w.toLowerCase()),
      'al',
      'bin',
      'ibn',
    ]);
  ...
  if (type.includes('diagnos') || type.includes('dx')) {
    ...
    for (const word of connectors) words.add(word);
    return words;
  }
  ...
  if (type.includes('status') || type.includes('evacuation')) {
    return new Set([
      ...VOCABULARY.ward_structure.map(w => w.toLowerCase()),
      ...connectors,
    ]);
  }
  ...
  if (type.includes('med') || type.includes('drug') || type.includes('rx')) {
    return new Set([
      ...VOCABULARY.medications.map(w => w.toLowerCase()),
      ...connectors,
    ]);
  }
